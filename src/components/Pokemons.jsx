...
import { useIntersectionObserver } from '../hooks/hookIntersectionObserver';
...
const Pokemons = () => {
    ...
    useEffect(() => {
        axios.get("https://pokeapi.co/api/v2/pokemon?limit=898").then(({ data }) => setAllPokemons(data.results)).catch((err) => console.log(err))
    }, []);
        ...
        <section className='p-4 py-5'>
            <form onSubmit={handleSubmit}>
                <div className='bg-white p-2 flex rounded-2xl text-lg'>
                    <input className="outline-none flex-1" type='text' placeholder='Buscá tu Pokémon' name='pokemonName' />
